...
int find_min(void)
{
    // Iniciate minimun number of votes to be equal to the first candidates vote count
    int min = candidates[0].votes;

    for (int i = 0; i < candidate_count; i++)
    {
        // If not eliminated and has less votes update
...
bool is_tie(int min)
{
    for (int i = 0; i < candidate_count; i++)
    {
        // Check if every candidate that is not eliminated has the same number of votes as the minimun
        if (!candidates[i].eliminated && candidates[i].votes != min)
        {
            return false;
        }
